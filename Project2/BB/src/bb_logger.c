...
void * logger(void * arg)
{
    ...
    if (FAILURE == open(name, O_RDONLY))
    {
        if (mkfifo(name, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH
                                                            | S_IWOTH))
        {
            string = "(Logger) [ERROR]: Creation of named pipe failed.\n";
            log_str(string);
            int8_t retvalue = FAILURE;
            pthread_exit(&retvalue);
        }
    ...
    if (!p_packet)
    {
        string = "(Logger) [ERROR]: Dynamic allocation of memory for comms packet failed...\n";
        log_str
        int8_t retvalue = FAILURE;
        pthread_exit(&retvalue);
    }
    ...
    while (!graceful_exit)
    {
        ...
        if (FAILURE == pipe_fd)
        {
            string = "(Logger) [ERROR]: Failed to open named pipe for read.\n";
            log_str(string);
            int8_t retvalue = FAILURE;
            pthread_exit(&retvalue);
        }
        ...
        {
            string = "(Logger) [ERROR]: Read from pipe failed.\n");
            log_str(string);
            int8_t retvalue = FAILURE;
            pthread_exit(&retvalue);
        }
