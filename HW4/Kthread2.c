...
*second thread on a timed interval through the kfifo.The second thread 
*takes data from the kfifo and prints it to the kernel logger every
* 100 ms.The info is the PID of the current, next and previous process.  
* 
...
unsigned char	buf[6];
    ...
    unsigned char       g[10];
    unsigned char       g1;
    unsigned long long int vc;
    unsigned long long int vp;
    unsigned long long int vn;
	 INIT_KFIFO(test);
	 INIT_KFIFO(test1);
	 INIT_KFIFO(test2);    	
...
/**
*@function: bus_timer_interrupt_init
*
...
int __init  bus_timer_interrupt_init(void) 
{
	...
	thread_10ms = kthread_create(writer, NULL, "writer");
